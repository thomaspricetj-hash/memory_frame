...
pub struct DeltaSlice {
    ...
    // Confidence-weighted delta strength (0â€“1)
...
impl DaxFrame {
    ...
    pub fn fold_frames(frames: &[FrameRecord]) -> Result<Self> {
        ...
        for slice in &base_frame.slices {
            let bytes = bincode::serialize(slice)
                .context("DaxFrame::fold_frames: bincode serialize base slice failed")?;
    ...
    /// Serialize DaxFrame to bytes using bincode.
    pub fn to_bytes(&self) -> Result<Vec<u8>> {
        let raw = bincode::serialize(self).context("DaxFrame::to_bytes: bincode serialize failed")?;
        Ok(raw)
    }

    /// Deserialize DaxFrame from bytes using bincode.
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        let df: DaxFrame = bincode::deserialize(bytes).context("DaxFrame::from_bytes: bincode deserialize failed")?;
        Ok(df)
    }
