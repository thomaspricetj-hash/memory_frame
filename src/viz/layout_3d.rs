...
impl FrameLayout3D {
    ...
    pub fn from_frame_record(frame: &FrameRecord) -> Self {
        ...
        for (i, slice) in slices.iter().enumerate() {
            ...
            // Confidence‑weighted radius
            let radius = base_radius * (0.8 + avg_conf * 0.4);

            // Depth (Z) based on slice ID hash — stable, deterministic
...
/// Deterministic Z‑depth based on slice ID hash.
fn depth_from_id(id: &str) -> f32 {
    ...
    id.hash(&mut h);
    let v = (h.finish() % 1000) as f32 / 1000.0; // 0.0–1.0
