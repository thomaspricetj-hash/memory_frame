...
//
// Patches applied:
// - Exported key types/functions as `pub` so they can be re-used by storage::mod
// - Switched logical serialization from bincode -> serde_cbor for compatibility
//   with dynamic types (serde_json::Value) used elsewhere.
// - Use bitdrop_v2 compress/decompress helpers (bd_compress / bd_decompress).
// - Replaced an internal call to `diagonal_weight` with `diagonal_weight_for_delta`
//   so this module does not depend on functions defined in other modules.
// - Added Context-style error messages where appropriate.
...
use serde_cbor;
use bitdrop_v2::{compress as bd_compress, decompress as bd_decompress};
...
impl FoldedFrame {
    ...
    pub fn fold(frame: &FrameRecord) -> Result<Self> {
        ...
        for (idx, slice) in frame.slices.iter().enumerate() {
            // Logical bytes (CBOR) for checksum + harmonic signature.
            let logical_bytes = serde_cbor::to_vec(slice)
                .map_err(|e| anyhow::anyhow!("fold: cbor serialize slice failed: {}", e))?;
            // Compressed payload via BitDrop-V2.
            let bytes = bd_compress(&logical_bytes);

            let mut hasher = DefaultHasher::new();
            logical_bytes.hash(&mut hasher);
    ...
    pub fn unfold(&self) -> Result<FrameRecord> {
        ...
        for (idx, fs) in self_iter_enumerate(&self.slices) {
            // Decompress via BitDrop-V2 to recover logical bytes.
            let logical_bytes = bd_decompress(&fs.data);
            ...
            let slice: SliceRecord = serde_cbor::from_slice(&logical_bytes)
                .map_err(|e| anyhow::anyhow!("unfold: cbor deserialize slice failed: {}", e))?;
            slices.push(slice);
        }
...
pub struct HybridBaseSlice {
    pub id: String,
    pub data: Vec<u8>,   // folded (CBOR) base slice
    pub checksum: u64,
    pub harmonic_sig: u64,
}
...
impl HybridDaxFrame {
    ...
    pub fn fold_frames(frames: &[FrameRecord]) -> Result<Self> {
        ...
        for (idx, slice) in base_frame.slices.iter().enumerate() {
            let bytes = serde_cbor::to_vec(slice)
                .map_err(|e| anyhow::anyhow!("hybrid fold: cbor serialize base slice failed: {}", e))?;
            ...
            for slice in &frame.slices {
                ...
                if let Some((base_index, base)) = matched {
                    let current_bytes = serde_cbor::to_vec(slice)
                        .map_err(|e| anyhow::anyhow!("hybrid fold: cbor serialize current slice failed: {}", e))?;
                    ...
                    let bytes = serde_cbor::to_vec(slice)
                        .map_err(|e| anyhow::anyhow!("hybrid fold: cbor serialize new base slice failed: {}", e))?;
                    ...
                    let diag_w =
                        diagonal_weight_for_slice(base_slices.len(), base_slices.len() + 1);
                    checksum ^= diag_w.to_bits() as u64;
    ...
    pub fn unfold_frame(&self, frame_index: usize) -> Result<FrameRecord> {
        // No checksum enforcement here; HybridDaxFrame is semantic, not bit-stable.
        ...
        if frame_index == 0 {
            ...
            for (_idx, b) in self_iter_enumerate(&self.base_slices) {
                let slice: SliceRecord = serde_cbor::from_slice(&b.data)
                    .map_err(|e| anyhow::anyhow!("hybrid unfold: cbor deserialize base slice failed: {}", e))?;
                slices.push(slice);
            }
        ...
        for (idx, base) in self_iter_enumerate(&self.base_slices) {
            ...
            let slice: SliceRecord = serde_cbor::from_slice(&current_bytes)
                .map_err(|e| anyhow::anyhow!("hybrid unfold: cbor deserialize reconstructed slice failed: {}", e))?;
            slices.push(slice);
        }
...
fn diagonal_semantic_match(
    ...
    // Use the module-local diagonal weight for deltas (no cross-module dependency).
    let w = diagonal_weight_for_delta(base_index, frame_index);
    w > 0.5
}
...
/// Simple XOR delta generator: returns bytes representing XOR of a and b (length = max(len(a), len(b)))
fn xor_delta_max(a: &[u8], b: &[u8]) -> Vec<u8> {
...
/// Apply XOR delta: returns a ^ delta (where delta was produced by xor_delta_max)
fn xor_apply_max(a: &[u8], delta: &[u8]) -> Vec<u8> {
    let max_len = a.len().max(delta.len());
    let mut out = Vec::with_capacity(max_len);
    for i in 0..max_len {
        let av = if i < a.len() { a[i] } else { 0 };
        let dv = if i < delta.len() { delta[i] } else { 0 };
        out.push(av ^ dv);
    }
    out
}
...
pub fn extract_confidence_from_bytes(bytes: &[u8]) -> f32 {
    if let Ok(sr) = serde_cbor::from_slice::<SliceRecord>(bytes) {
