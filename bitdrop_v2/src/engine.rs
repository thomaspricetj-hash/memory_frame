...
use zstd::bulk::compress as zstd_enc;
...
impl BitDrop3DEngine {
    ...
    pub fn decode(&self, blob: &[u8]) -> Vec<u8> {
        ...
        match mode {
            ...
            1 => {
                // Use streaming decode which does not require preallocating a destination buffer.
    ...
    pub fn decode_origami(&self, blob: &[u8]) -> Vec<u8> {
        ...
        if body.len() < rubik_len_offset + 2 {
            return Vec::new();
        }
...
// Raw sparse diff
// ------------------------------------------------------------
fn compute_sparse_diff(base: &[u8], token: &[u8]) -> Vec<u8> {
    let n = base.len().min(token.len());
    let mut out = Vec::new();
    let mut pairs = Vec::new();

    for i in 0..n {
        if base[i] != token[i] {
            pairs.push((i as u32, token[i]));
        }
    }
...
fn apply_sparse_diff(base: &[u8], diff: &[u8]) -> Vec<u8> {
    if diff.len() < 4 {
        return base.to_vec();
    }
    ...
    if offset + 4 > diff.len() {
        return base.to_vec();
    }
    let count = u32::from_be_bytes(diff[offset..offset + 4].try_into().unwrap()) as usize;
    offset += 4;
    ...
    for _ in 0..count {
        ...
        let idx = u32::from_be_bytes(diff[offset..offset + 4].try_into().unwrap()) as usize;
        offset += 4;
        if offset >= diff.len() {
            break;
        }
...
// Token sparse diff
// ------------------------------------------------------------
fn compute_token_diff(master: &[u32], token: &[u32]) -> Vec<u8> {
    let n = master.len().min(token.len());
    let mut out = Vec::new();
    let mut pairs = Vec::new();
...
fn apply_token_diff(master: &[u32], diff: &[u8]) -> Vec<u32> {
    if diff.len() < 4 {
    ...
    for _ in 0..count {
        ...
        let val = u32::from_be_bytes(diff[offset..offset + 4].try_into().unwrap());
        offset += 4;
...
// ------------------------------------------------------------
// DAX frame builder / parser WITH MODE BYTE
// mode: 0 = raw-DAX, 2 = token-DAX
// ------------------------------------------------------------
fn dax_build_frame(master: &DaxToken, deltas: &[DaxDelta], mode: u8) -> Vec<u8> {
    let mut out = Vec::new();

    out.extend_from_slice(DAX_MAGIC);
    out.push(mode);

    let name_bytes = master.name.as_bytes();
    out.extend_from_slice(&(name_bytes.len() as u16).to_be_bytes());
    out.extend_from_slice(name_bytes);

    out.extend_from_slice(&(master.data.len() as u32).to_be_bytes());
    out.extend_from_slice(&master.data);
    ...
    for d in deltas {
        ...
        let bb = d.base_name.as_bytes();
        out.extend_from_slice(&(bb.len() as u16).to_be_bytes());
        out.extend_from_slice(bb);

        out.extend_from_slice(&(d.diff.len() as u32).to_be_bytes());
        out.extend_from_slice(&d.diff);
    }

    out
}

fn dax_parse_frame(frame: &[u8]) -> Option<(u8, DaxToken, Vec<DaxDelta>)> {
    // robust parsing with bounds checks to avoid panics
    if frame.len() < 5 || &frame[0..4] != DAX_MAGIC {
        return None;
    }
    ...
    let name_len = u16::from_be_bytes(frame[offset..offset + 2].try_into().unwrap()) as usize;
    offset += 2;

    if offset + name_len > flen {
        return None;
    }
    let name = String::from_utf8_lossy(&frame[offset..offset + name_len]).to_string();
    offset += name_len;

    if offset + 4 > flen {
        return None;
    }
    let data_len = u32::from_be_bytes(frame[offset..offset + 4].try_into().unwrap()) as usize;
    offset += 4;

    if offset + data_len > flen {
        return None;
    }
    let data = frame[offset..offset + data_len].to_vec();
    offset += data_len;

    if offset + 4 > flen {
        return None;
    }
    let delta_count = u32::from_be_bytes(frame[offset..offset + 4].try_into().unwrap()) as usize;
    offset += 4;
    ...
    for _ in 0..delta_count {
        ...
        let nl = u16::from_be_bytes(frame[offset..offset + 2].try_into().unwrap()) as usize;
        offset += 2;

        if offset + nl > flen {
            return None;
        }
        let name = String::from_utf8_lossy(&frame[offset..offset + nl]).to_string();
        offset += nl;

        if offset + 2 > flen {
            return None;
        }
        let bl = u16::from_be_bytes(frame[offset..offset + 2].try_into().unwrap()) as usize;
        offset += 2;

        if offset + bl > flen {
            return None;
        }
        let base = String::from_utf8_lossy(&frame[offset..offset + bl]).to_string();
        offset += bl;

        if offset + 4 > flen {
            return None;
        }
        let dl = u32::from_be_bytes(frame[offset..offset + 4].try_into().unwrap()) as usize;
        offset += 4;

        if offset + dl > flen {
            return None;
        }
        let diff = frame[offset..offset + dl].to_vec();
        offset += dl;

        deltas.push(DaxDelta { name, base_name: base, diff });
    }

    Some((mode, DaxToken { name, data }, deltas))
}

// ------------------------------------------------------------
// Hybrid Auto-Mode DAX Encoder
// ------------------------------------------------------------
pub fn bitdrop_encode_dax_auto(
    raw: Vec<(String, Vec<u8>)>,
    _cubes: Option<Vec<(String, Vec<Vec<u8>>)>>,
    tokens: Option<Vec<(String, Vec<u32>)>>,
) -> Vec<u8> {
    // Token-DAX
    if let Some(tok) = tokens {
        // master tokens (u32) are in tok[0].1
        let (master_name, master_tokens) = &tok[0];

        let deltas: Vec<DaxDelta> = tok
            .par_iter()
            .skip(1)
            .map(|(name, t)| {
                let diff = compute_token_diff(master_tokens, t);
                DaxDelta {
                    name: name.clone(),
                    base_name: master_name.clone(),
                    diff,
                }
            })
            .collect();

        // If original raw master bytes are available, prefer storing them as the master payload
        // so that decode returns the original master bytes (fixes small-text correctness checks).
        let master_data_bytes: Vec<u8> = if !raw.is_empty() {
            raw[0].1.clone()
        } else {
            master_tokens.iter().flat_map(|v| v.to_be_bytes()).collect()
        };

        let master = DaxToken {
            name: master_name.clone(),
            data: master_data_bytes,
        };

        let frame = dax_build_frame(&master, &deltas, 2);
        return zstd_enc(&frame, 1).unwrap();
    }
    ...
    let (master_name, master_data) = &raw[0];

    let deltas: Vec<DaxDelta> = raw
        .par_iter()
        .skip(1)
        .map(|(name, d)| {
            let diff = compute_sparse_diff(master_data, d);
            DaxDelta {
                name: name.clone(),
                base_name: master_name.clone(),
                diff,
    ...
    let frame = dax_build_frame(&master, &deltas, 0);
    zstd_enc(&frame, 1).unwrap()
}

// ------------------------------------------------------------
// Hybrid Auto-Mode DAX Decoder
// ------------------------------------------------------------
pub fn bitdrop_decode_dax_auto(blob: &[u8]) -> Vec<(String, Vec<u8>)> {
    // Use streaming decode which avoids destination-size estimation issues.
    let frame = match zstd_stream_dec(blob) {
        Ok(decoded) => decoded,
        Err(_) => return Vec::new(),
    };

    let (mode, master, deltas) = match dax_parse_frame(&frame) {
        Some(v) => v,
        None => return Vec::new(),
    };
    ...
    match mode {
        2 => {
            // Token-DAX decode
            // If master.data length is a multiple of 4, interpret as tokens; otherwise treat as raw bytes.
            if master.data.len() % 4 == 0 {
                let master_tokens = master
                    .data
                    .chunks_exact(4)
                    .map(|b| u32::from_be_bytes(b.try_into().unwrap()))
                    .collect::<Vec<u32>>();

                // Return master as original bytes (if they were stored as raw bytes, master.data already contains them).
                out.push((master.name.clone(), master.data.clone()));

                for d in deltas {
                    let t = apply_token_diff(&master_tokens, &d.diff);
                    let bytes = t.iter().flat_map(|v| v.to_be_bytes()).collect::<Vec<u8>>();
                    out.push((d.name, bytes));
                }
            } else {
                // Master stored as raw bytes (preferred path when raw was provided at encode time).
                out.push((master.name.clone(), master.data.clone()));
                for d in deltas {
                    // deltas still contain token diffs; attempt to apply them if possible.
                    // If master.data cannot be interpreted as tokens, we return diffs as-is by applying sparse diff fallback.
                    // Try to interpret master.data as tokens if length is multiple of 4; otherwise, return empty token outputs.
                    if master.data.len() % 4 == 0 {
                        let master_tokens = master
                            .data
                            .chunks_exact(4)
                        ...
                        // Can't reconstruct token payloads; return empty vectors to indicate failure to reconstruct tokens.
                        out.push((d.name, Vec::new()));
                    }
                }
            }
        }
        ...
        _ => {
            ...
            for d in deltas {
                let data = apply_sparse_diff(&master.data, &d.diff);
                out.push((d.name, data));
            }
        }
    }
