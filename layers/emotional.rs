...
pub struct EmotionalOutput {
    pub value: f32,              // normalized 0â€“1 emotional intensity
...
impl LayerTrait for EmotionalLayer {
    ...
    fn encode(input: Self::Input) -> Result<Self::Output> {
        if !input.is_finite() {
            return Err(anyhow!("EmotionalLayer: nonâ€‘finite emotional score"));
        }

        // Normalize into 0â€“1 range
        let value = input.clamp(0.0, 1.0);

        // Confidence heuristic: midâ€‘range values are more stable
...
/// - midâ€‘range values (~0.5) are more stable
