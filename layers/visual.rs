...
impl LayerTrait for VisualLayer {
    ...
    fn encode(input: Self::Input) -> Result<Self::Output> {
        ...
        // Step 1: Repair invalid pixel values (should be 0â€“255)
...
/// Normalize pixel buffer into grayscale (0â€“255).
...
fn normalize_grayscale(pixels: &[u8]) -> Vec<u8> {
    ...
    if max == min {
        return vec![128; pixels.len()]; // flat image â†’ mid-gray
    }
...
fn compute_noise_level(pixels: &[u8]) -> f32 {
    ...
    // Normalize variance into 0â€“1 range
...
fn compute_histogram_confidence(pixels: &[u8]) -> f32 {
    ...
    // Normalize entropy into 0â€“1 range
